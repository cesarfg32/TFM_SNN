...
class RehearsalConfig:
    ...
    # NUEVO: parámetros SÓLO para el replay loader (evitar duplicar presión de memoria)
    replay_num_workers: Optional[int] = 0
    replay_pin_memory: bool = False
    replay_persistent_workers: bool = False
    # opcional: por si quieres cortar el último batch
    replay_drop_last: bool = False
    # opcional: barajar el replay (por defecto sí)
...
class RehearsalMethod:
    ...
    def after_task(self, model, train_loader: DataLoader, val_loader: DataLoader) -> None:
        if self._last_task_loader is not None:
            ...
            added = self.buffer.add_from_loader(self._last_task_loader)  # ← int
            now = len(self.buffer)
    ...
    def prepare_train_loader(self, train_loader: DataLoader) -> DataLoader | _ReplayMixLoader:
        ...
        # Construye replay_loader con parámetros conservadores por defecto
        replay_ds = self.buffer.as_dataset()

        # Derivar num_workers del task loader si no se fijó explícito
        tl_workers = int(getattr(train_loader, "num_workers", 0) or 0)
        rep_workers = self.cfg.replay_num_workers
        if rep_workers is None:
            # por defecto: reducir a la mitad (redondeo a la baja)
            rep_workers = max(0, tl_workers // 2)
        # pero en general, para smoke/OOM: 0 es lo más seguro
        if rep_workers < 0:
            rep_workers = 0
        ...
        replay_loader = DataLoader(
            replay_ds,
            batch_size=rep_bs,
            shuffle=self.cfg.replay_shuffle,
            num_workers=rep_workers,
            pin_memory=self.cfg.replay_pin_memory,
            persistent_workers=self.cfg.replay_persistent_workers if rep_workers > 0 else False,
            drop_last=self.cfg.replay_drop_last,
        )

        if getattr(self, "replay_verbose", False):
            msg = (f"[Rehearsal] mix: task_bs={task_bs} | replay_bs={rep_bs} | total_bs={total_bs} "
                   f"| buf_len={len(self.buffer)} | rep_workers={rep_workers} "
                   f"| rep_pin={self.cfg.replay_pin_memory} "
                   f"| rep_persist={self.cfg.replay_persistent_workers}")
            if getattr(self, "replay_show_shapes", False):
                try:
                    xb_t, _ = next(iter(train_loader))
                    xb_r, _ = next(iter(replay_loader))
