...
class EWC:
    ...
    def __init__(self, model: nn.Module, cfg: EWCConfig, loss_fn: Optional[nn.Module] = None, device: Optional[torch.device] = None):
        self.model = model
        self.cfg   = cfg
    ...
    def estimate_fisher(self, loader) -> int:
        ...
        fisher = {n: torch.zeros_like(p, device=p.device) for n, p in self.model.named_parameters() if p.requires_grad}
        ...
        with torch.enable_grad():
            for x, y in loader:
                ...
                y_hat = _forward_with_cached_orientation(self.model, x, y, self.device, use_amp=True, phase_hint=hint, phase="fisher")
...
class EWCMethod(BaseMethod):
    ...
    def __init__(self, model: nn.Module, lambd: float, fisher_batches: int, loss_fn: nn.Module, device: torch.device):
        ...
        self.impl = EWC(model, EWCConfig(lambd=float(lambd), fisher_batches=int(fisher_batches)), loss_fn=loss_fn, device=device)
