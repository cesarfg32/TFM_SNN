...
@dataclass
class SASNNConfig:
    # Dónde enganchar (ruta exacta en model.named_modules(); usa "auto" para 1ª Linear/Conv2d)
    attach_to: str
    k: float = 8.0
    tau: float = 32.0
    vt_scale: float = 1.33
...
class SASNN(BaseMethod):
    ...
    def __init__(self, model: nn.Module, cfg: SASNNConfig, device: Optional[DeviceLike] = None, loss_fn=None):
        super().__init__(device=(torch.device(device) if device is not None else next(model.parameters()).device),
                         loss_fn=loss_fn)
        self.model = model
        self.cfg = cfg
    ...
    def detach(self) -> None:
        try:
            if self._hook_handle is not None: self._hook_handle.remove()
        finally:
            self._hook_handle = None
            self._target = None  # type: ignore
            self._tr = None; self._ema = None; self._counters = None; self._vt_bias = None
    ...
    # ---- hook principal (sin @no_grad; máscara fuera del grafo) ----
    def _forward_hook(self, module: nn.Module, inputs: Tuple[Tensor, ...], output: Tensor) -> Tensor:
        ...
        if out.dim() < 2:
            B = out.shape[0]; out = out.view(B, 1)
        B, C = out.shape[:2]; spatial = out.shape[2:]
        ...
        with torch.no_grad():
            ...
            if self.cfg.assume_binary_spikes: S = (S > 0).to(out.dtype)
            ...
            if is_train_mode:
                ...
                self._counters.add_(S); self._t_seen += 1
            ...
            if kk >= N: sel_idx = torch.arange(N, device=self.device)
            else:       sel_idx = torch.topk(score, k=kk, dim=0, largest=True, sorted=False).indices
            mask_neurons = torch.zeros(N, dtype=out.dtype, device=self.device); mask_neurons[sel_idx] = 1.0
    ...
    def _resolve_k(k: float, N: int) -> int:
        if k < 1.0: kk = max(1, int(round(float(k) * N)))
        else:       kk = int(round(k))
    ...
    def _reduce_activity(self, out: Tensor, flatten_spatial: bool) -> Tensor:
        ...
        B = out.shape[0]; flat = out.view(B, -1)
    ...
    def _apply_mask(self, out: Tensor, mask_neurons: Tensor, flatten_spatial: bool) -> Tensor:
        ...
        HW = int(torch.prod(torch.tensor(spatial)).item()) if spatial else 1
        N = C * HW
        if N != mask_neurons.numel(): return out
        if HW == 1:
